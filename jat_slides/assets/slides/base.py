...
def add_title_slide(pres: PresentationType, title_layout: SlideLayout) -> None:
    ...
    placeholders[2].text = format_date(datetime.date.today(), locale="es", format="long")


def add_section_slide(pres: PresentationType, section_layout: SlideLayout, section_name: str) -> None:
    section = pres.slides.add_slide(section_layout)
    section.shapes[0].text = section_name


def add_highlighted_text_to_frame(frame: TextFrame, *, text_left: str, highlight: float, text_right: str) -> None:
...
def add_lost_pop_slide(pres: PresentationType, layout: SlideLayout, lost_pop_frac: float, picture_path: Path) -> None:
...
def add_built_slide(
    ...
    *,
    ...
    built_change = (built_area_df.loc[2020] - built_area_df.loc[2000]) / built_area_df.loc[2000] + 1
    urban_change = (urban_area_df.loc[2020] - urban_area_df.loc[2000]) / urban_area_df.loc[2000] + 1
    ...
    text_arr = np.array([
        ["", "Millones de habitantes", "Superficie  construida (km²)", "Superficie urbana (km²)"],
        ["2000", f"{pop_df.loc[2000]:.2f}", f"{built_area_df.loc[2000]:.1f}", f"{urban_area_df.loc[2000]:.1f}"],
        ["2020", f"{pop_df.loc[2020]:.2f}", f"{built_area_df.loc[2020]:.1f}", f"{urban_area_df.loc[2020]:.1f}"],
...
def slides(
    ...
    for zone in zones_resource.wanted_zones:
        ...
        lost_pop_frac = lost_pop_after_2000.loc[lost_pop_after_2000["zone"] == zone, "lost"].item()
        lost_pop_fig = figure_path / f"mesh/{zone}.png"
        add_lost_pop_slide(pres, layouts["pop"], lost_pop_frac, lost_pop_fig)

        built_after_frac = built_after_2000.loc[built_after_2000["zone"] == zone, "built"].item()
        built_year_fig = figure_path / f"built/{zone}_r.png"
        add_built_slide(pres, layouts["built"], built_after_frac, pop_df=pop_df[zone], built_area_df=built_df[zone], urban_area_df=built_urban_df[zone], picture_path=built_year_fig)
