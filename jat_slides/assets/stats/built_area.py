...
def load_built_area_rasters_factory(year: int):
    ...
    def _op(path_resource: PathResource, bounds: dict[int, list]) -> tuple[np.ndarray, Affine]:
...
load_built_area_rasters_ops = {year: load_built_area_rasters_factory(year) for year in range(1975, 2021, 5)}


# pylint: disable=unused-argument
@op(out=Out(io_manager_key="csv_manager"))
def reduce_area_rasters(rasters: list[np.ndarray], transforms: list[Affine]) -> pd.DataFrame:
    out = []
...
@op
def get_bounds(agebs_1990: gpd.GeoDataFrame, agebs_2000: gpd.GeoDataFrame, agebs_2010: gpd.GeoDataFrame, agebs_2020: gpd.GeoDataFrame) -> dict[int, list]:
    bounds = {}
...
def built_area(agebs_1990: gpd.GeoDataFrame, agebs_2000: gpd.GeoDataFrame, agebs_2010: gpd.GeoDataFrame, agebs_2020: gpd.GeoDataFrame) -> pd.DataFrame:
    rasters, transforms = [], []
